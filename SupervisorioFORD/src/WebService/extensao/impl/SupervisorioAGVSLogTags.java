...
public class SupervisorioAGVSLogTags implements Command{

	private ConvertPAGinHTML cph;
	private String[] files = {PathFilesPAG.logTags};
	...
	public void execute(Request req, Response resp) throws Exception {
		...
		if(!Login.login(req, resp, Login.tokenLogTags)) {
		...
		tags.add(Keys.keyGenerateDashboard, mt.methodGenerateDashboardLogTags);
		tags.add(Keys.keyGenerateUser, mt.methodGenerateNomeUsuario);
		tags.add(Keys.keyGenerataTableLogTags, mt.methodGerarTabelaLogTags);
