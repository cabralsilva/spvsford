...
(function(document, window, $) {
	...
	(function() {
		$(document).ready(function() {
			...
			$('#condicoesMesh').DataTable(options);
						});
	})();
	...
	(function($) {
		var EditableTable = {
			...
			rowEdit : function($row) {
				...
				$row.children('td').each(function(i) {
					...
					if ($this.hasClass('actions')) {
						...
						if (i == 0) {
							$this.html('<input disabled type="number" id="'
											+ i
											+ '" class="form-control input-block" value="'
											+ data[i] + '"/>');
							idAction = data[i];
							console.log($this.html());
						} else if (i == 2) {
							console.log(rowMesh());
							$this.html('<select  id="selectMesh" class="form-control" data-plugin="select">' + rowMesh() + '</select>');
							valueMesh = data[i];
							console.log($this.html());
						} else {
				...
				var element = document.getElementById('selectMesh');
				if(valueMesh != '') {
					for (var i = 0; i < element.options.length; i++) {
					    if (element.options[i].text == valueMesh) {
			...
			rowSave : function($row) {
				...
				var link = '/ActionCadastroSemaforo?' + 'action=' + action + '&id='
						+ document.getElementById("0").value + '&nome='
						+ document.getElementById("1").value + '&meshserial='
						+ document.getElementById("selectMesh").value;
				...
				if (idReturn > 0) {
					...
					values = $row.find('td').map(function() {
						...
						if ($this.hasClass('actions')) {
							...
							if(i == 3) {
								e = document.getElementById("selectMesh");
			...
			rowRemove : function($row) {
				...
				var link = "/ActionCadastroSemaforo?action=delete&id=" + id;
				if (action == "delete") {
					if (enviarDados(link)) {
