...
(function(document, window, $) {
  ...
  (function($) {
    var EditableTable = {
      ...
      events: function() {
        ...
        this.$table
		  ...
		  .on('click', 'a.libera-row', function(e) {
            e.preventDefault();
            var $row = $(this).closest('tr');
            bootbox.dialog({
              message: "Tem certeza de que deseja Liberar esta Cruzamento?",
              ...
              buttons: {
                danger: {
                  ...
                  callback: function() {
					...
					var link = "/ActionCadastroCruzamentos?action=resetCruzamento&id=" + id;
			        enviarDados(link);
                  }
                },
                main: {
      ...
      rowAdd: function() {
        ...
        actions = [
		  '<a href="#" class="btn btn-sm btn-icon btn-pure btn-default on-default libera-row" data-toggle="tooltip" data-original-title="Save"><i class="icon wb-unlock" aria-hidden="true"></i></a>',
	...
	var link = '/ActionCadastroCruzamentos?action=' + action + '&idOld=' + idAction + '&nome=' + document.getElementById("0").value + '&descricao=' + document.getElementById("1").value;
	if(enviarDados(link)){
        var _self = this,
          $actions,
          values = [];
      ...
      rowRemove: function($row) {
		...
		var id = $row.get(0).cells[0].innerHTML;
		var link = "/ActionCadastroCruzamentos?action=delete&id=" + id;
		if(action == "delete") {
			if(enviarDados(link)) {
