...
public class SupervisorioAGVSRelatorioEventosAGVS implements Command{

	private ConvertPAGinHTML cph;
	private String[] files = {PathFilesPAG.relatoriosEventosAGVS};
	...
	public void execute(Request req, Response resp) throws Exception {
		...
		if(!Login.login(req, resp, Login.tokenRelatorioEventosAGVS)) {
		...
		tags.add(Keys.keyGenerateDashboard, mt.methodGenerateDashboardRelatorioEventosAGVS);
		tags.add(Keys.keyGenerateUser, mt.methodGenerateNomeUsuario);
		tags.add(Keys.keyGerarTabelaRelatorioEventoAGVS, mt.methodGerarTabelaRelatorioEventoAGVS);
