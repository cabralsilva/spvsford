  ...
  (function($) {
    var EditableTable = {
      ...
      rowEdit: function($row) {
		...
		$row.children('td').each(function(i) {
          ...
          if ($this.hasClass('actions')) {
			...
			if(i == 0) { 
			  ...
			  $this.html('<select  id="selectTags" class="form-control" data-plugin="select">' + rowTags() + '</select>');
              valueTag = data[i];
			} else {
              $this.html('<input type="text" id="' + i + '" class="form-control input-block" value="' + data[i] + '"/>');
            }
		...
		var element = document.getElementById('selectTags');
		if(valueTag != '')
	...
	var link = '/ActionCadastroTempoTagsParado?action=' + action + '&idOld=' + idAction + '&nome=' + document.getElementById("0").value + '&tag=' + document.getElementById("selectTags").value;
	if(enviarDados(link)){
        var _self = this,
          $actions,
          values = [];
      ...
      rowRemove: function($row) {
		...
		var link = "/ActionCadastroTempoTagsParado?action=delete&id=" + id;
		if(action == "delete") {
			if(enviarDados(link)) {
