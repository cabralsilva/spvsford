	...
	(function($) {
		var EditableTable = {
			...
			rowAdd : function() {
				...
				;
				this.rowEdit($row);
				this.datatable.order([ 0, 'asc' ]).draw(); // always show
															// fields
				$('#select6').select2({
					  multiple: true,					  
					});
			...
			rowEdit : function($row) {
				...
				$row.children('td').each(function(i) {
					...
					if ($this.hasClass('actions')) {
						...
						if (i == 0) {
							$this
									.html('<input required type="number" min="1" id="'
											+ i
											+ '" style="width: 100%;" class="form-control input-block" value="'
											+ data[i] + '"/>');
							idAction = data[i];
						} 
						else if (i == 2) {
							...
							$this.html('<input type="number" min="1" id="'+ i + '" required class="form-control input-block" style="width: 100%;" value="' + data[i] + '"/>');
						} else if (i == 6) {		                    
							$this.html('<select id="select'+i+'" class="form-control select2-primary">' + rowBaias() + '</select>');
						} else {
							$this.html('<input type="text" id="'+ i + '" required class="form-control input-block" style="width: 100%;" value="' + data[i] + '"/>');
						}
					}
			...
			rowSave : function($row) {
				...
				$row.children('td').children('input').each(function(i) {
					if (!document.getElementById(i).validity.valid){
						switch(i){
							...
							case 4:
							case 5:
								menssagemErro("Numero de Entradas/Saídas inválido");
								break;
						}
						valid = false;
					}
				...
				var selections = $('#select6').select2('data');
				var idBaias = "";
				for (var i in selections){
					idBaias += selections[i].id;
					if (i < selections.length-1) idBaias+=",";
				}
				var link = '/ActionCadastroMesh?'
						+ 'action=' + action 
						...
						if ($this.hasClass('actions')) {
							...
							if(i == 1) {
								...
								for (var j in selections){
									baias += selections[j].text;
									if (j < selections.length-1) baias+=",";
								}
								return baias;
							}
			...
			rowRemove : function($row) {
				...
				var link = "/ActionCadastroMesh?action=delete&id=" + id;
				if (action == "delete") {
					if (enviarDados(link)) {
