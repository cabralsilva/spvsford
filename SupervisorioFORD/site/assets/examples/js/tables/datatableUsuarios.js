...
(function(document, window, $) {
  ...
  (function($) {
      ...
      rowEdit: function($row) {
		...
		$row.children('td').each(function(i) {
          ...
          if ($this.hasClass('actions')) {
			...
			if(i == 1) {
              $this.html('<input type="password" id="' + i + '" class="form-control input-block" value="' + data[i] + '"/>');
			} else if(i == 4) {
              $this.html('<select  id="selectPermissao" class="form-control" data-plugin="select">' + rowConboboxPermissao() + '</select>');
              valuePermissao = data[i];
			} else {
				if(i == 0) {
					idAction = data[i];
				}
		...
		var element1 = document.getElementById('selectPermissao');
		if(valuePermissao != '')
			element1.value = valuePermissao;
	...
	var link = '/ActionUsuario?action=' + action + '&id=' + idAction + '&login=' + document.getElementById("0").value + '&password=' + document.getElementById("1").value + '&nome=' + document.getElementById("2").value + '&email=' + document.getElementById("3").value + '&permissao=' + document.getElementById("selectPermissao").value;
	if(enviarDados(link)){
		var _self = this,
          $actions,
          values = [];
        ...
        values = $row.find('td').map(function() {
          ...
          if ($this.hasClass('actions')) {
			...
			var t1 = document.getElementById("selectPermissao");
			var txt = t1.options[t1.selectedIndex].text
			if(txt == 'Personalizado') {
			...
			if(i == 5) {
				var e1 = document.getElementById("selectPermissao");
      ...
      rowRemove: function($row) {
        ...
        var link = "/ActionUsuario?action=delete&id=" + id;
		if(action == "delete") {
			if(enviarDados(link)) {
