...
public class SupervisorioAGVSCadastroSupermercados implements Command {

	private ConvertPAGinHTML cph;
	private String[] files = { PathFilesPAG.cadastroSupermercados };
	...
	public void execute(Request req, Response resp) throws Exception {
		...
		if (!Login.login(req, resp, Login.tokenCadastroSupermercados)) {
		...
		tags.add(Keys.keyGenerateDashboard, mt.methodGenerateDashboardCadastroSupermercados);
		tags.add(Keys.keyGenerateUser, mt.methodGenerateNomeUsuario);
		tags.add(Keys.keyGerarTabelaSupermercados, mt.methodGerarTabelaSupermercados);
