...
public class SupervisorioAGVSCadastroSemaforo implements Command{

	private ConvertPAGinHTML cph;
	private String[] files = {PathFilesPAG.cadastroSemaforo};
	...
	public void execute(Request req, Response resp) throws Exception {
		...
		if(!Login.login(req, resp, Login.tokenCadastroSemaforos)) {
		...
		tags.add(Keys.keyGenerateDashboard, mt.methodGenerateDashboardCadastroSemaforo);
		tags.add(Keys.keyGenerateUser, mt.methodGenerateNomeUsuario);
		tags.add(Keys.keyGerarTabelaSemaforo, mt.methodGerarTabelaSemaforo);
		tags.add(Keys.keyListMesh, mt.methodListMesh);
