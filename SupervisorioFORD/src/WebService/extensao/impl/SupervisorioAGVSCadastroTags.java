...
public class SupervisorioAGVSCadastroTags implements Command{

	private ConvertPAGinHTML cph;
	private String[] files = {PathFilesPAG.cadastroTags};
	...
	public void execute(Request req, Response resp) throws Exception {
		...
		if(!Login.login(req, resp, Login.tokenCadastroTags)) {
		...
		tags.add(Keys.keyGenerateDashboard, mt.methodGenerateDashboardCadastroTags);
		tags.add(Keys.keyGenerateUser, mt.methodGenerateNomeUsuario);
		tags.add(Keys.keyGerarTabelaTags, mt.methodGerarTabelaTags);
