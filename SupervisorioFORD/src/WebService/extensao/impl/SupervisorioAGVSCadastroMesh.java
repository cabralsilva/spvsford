...
public class SupervisorioAGVSCadastroMesh implements Command{

	private ConvertPAGinHTML cph;
	private String[] files = {PathFilesPAG.cadastroMesh};
	...
	public void execute(Request req, Response resp) throws Exception {
		...
		if(!Login.login(req, resp, Login.tokenCadastroMesh)) {
		...
		tags.add(Keys.keyGenerateDashboard, mt.methodGenerateDashboardCadastroMesh);
		tags.add(Keys.keyGenerateUser, mt.methodGenerateNomeUsuario);
		tags.add(Keys.keyGerarTabelaMesh, mt.methodGerarTabelaMesh);
		tags.add(Keys.keyGerarComboboxBaias, mt.methodGerarComboboxBaias);
