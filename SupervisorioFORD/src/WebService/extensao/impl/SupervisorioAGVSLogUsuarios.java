...
public class SupervisorioAGVSLogUsuarios implements Command{

	private ConvertPAGinHTML cph;
	private String[] files = {PathFilesPAG.logUsuarios};
	...
	public void execute(Request req, Response resp) throws Exception {
		...
		if(!Login.login(req, resp, Login.tokenLogUsuarios)) {
		...
		tags.add(Keys.keyGenerataTableLogUsuarios, mt.methodGenerataTableLogUsuarios);
		tags.add(Keys.keyGenerateDashboard, mt.methodGenerateDashboardLogUsuarios);
	...
	private void Home(Request req, Response resp) {
		PrintStream out = resp.getPrintStream();
		out.println(cph.getHtml(PathFilesPAG.logUsuarios));
		out.flush();
	}
