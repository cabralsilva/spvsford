      ...
      rowEdit: function($row) {
        ...
        $row.children('td').each(function(i) {
          ...
          if ($this.hasClass('actions')) {
			...
			if(i == 0) { 
			  $this.html('<input type="text" disabled id="' + i + '" class="form-control input-block" value="' + data[i] + '"/>');
              idAction = data[i];
			}else if(i == 1) { 
			  $this.html('<input type="text" required id="' + i + '" class="form-control input-block" value="' + data[i] + '"/>');
              idAction = data[i];
			}
			else if(i == 2) {
			  $this.html('<select  id="selectSemaforo" class="form-control" data-plugin="select">' + rowSemaforos() + '</select>');
              valueSemaforo = data[i];
			} else if(i == 3) {
			  $this.html('<select  id="selectTag" class="form-control" data-plugin="select">' + rowTags() + '</select>');	
              valueTag = data[i];
			} else if(i == 4) {
			  $this.html('<select  id="selectTipo" class="form-control" data-plugin="select">' + rowTipos() + '</select>');	
              valueTipo = data[i];
			} else {
        ...
        var element = document.getElementById('selectSemaforo');
        if(valueSemaforo != '') {
			for (var i = 0; i < element.options.length; i++) {
			    if (element.options[i].text == valueSemaforo) {
...
//		var element = document.getElementById('selectSemaforo');
//		if(valueSemaforo != '')
//			element.value = valueSemaforo;
		
		var element1 = document.getElementById('selectTipo');
		if(valueTipo != '')
			element1.value = valueTipo;
		
		var element2 = document.getElementById('selectTag');
		if(valueTag != '') {
			var k;
			for(k = 0; k < element2.childNodes.length; k++) {
				if(element2.childNodes[k].text == valueTag) {
					element2.value = element2.childNodes[k].value;
				}
			}
		}
      ...
      rowSave: function($row) {
    	  var valid = true;
    	  $row.children('td').children('input').each(function(i) {
    	  ...
    	  var link = '/ActionCadastroTagsSemaforo?action=' + action 
			+ '&id=' + document.getElementById("0").value 
			+ '&nome=' + document.getElementById("1").value 
			+ '&tipo=' + document.getElementById("selectTipo").value 
			+ '&epc=' + document.getElementById("selectTag").value 
			+ '&semaforo=' + document.getElementById("selectSemaforo").value;
    	  ...
    	  if (idReturn > 0) {
    		  ...
    		  values = $row.find('td').map(function() {
    			  ...
    			  if ($this.hasClass('actions')) {
    				  ...
    				  if(i == 3) {
    					  e = document.getElementById("selectSemaforo");
    					  return e.options[e.selectedIndex].text;
    				  }
    				  if(i == 4) {
    					  e = document.getElementById("selectTag");
						  return e.options[e.selectedIndex].text;
    				  }
      ...
      rowRemove: function($row) {
		...
		var link = "/ActionCadastroTagsSemaforo?action=delete&id=" + id;
		if(action == "delete") {
			if(enviarDados(link)) {
