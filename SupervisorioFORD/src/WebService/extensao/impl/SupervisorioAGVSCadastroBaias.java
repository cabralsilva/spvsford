...
public class SupervisorioAGVSCadastroBaias implements Command{

	private ConvertPAGinHTML cph;
	private String[] files = {PathFilesPAG.cadastroBaias};
	...
	public void execute(Request req, Response resp) throws Exception {
		...
		if(!Login.login(req, resp, Login.tokenCadastroBaias)) {
...
//methodGerarTabelaRotas
		...
		tags.add(Keys.keyGenerateDashboard, mt.methodGenerateDashboardCadastroBaias);
		tags.add(Keys.keyGenerateUser, mt.methodGenerateDashboardCadastroBaias);
		tags.add(Keys.keyGerarTabelaBaias, mt.methodGerarTabelaBaias);
		tags.add(Keys.keyGerarComboboxTipoBaias, mt.methodGerarComboboxTiposBaias);
