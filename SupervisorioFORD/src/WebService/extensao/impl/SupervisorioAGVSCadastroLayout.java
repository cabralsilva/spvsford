...
public class SupervisorioAGVSCadastroLayout implements Command {

	private ConvertPAGinHTML cph;
	private String[] files = { PathFilesPAG.cadastroLayout };
	...
	public void execute(Request req, Response resp) throws Exception {
		...
		if (!Login.login(req, resp, Login.tokenCadastroLayout)) {
...
//		tags.add(Keys.keyGenerateDashboard, mt.methodGenerateDashboardCadastroLayou);
		tags.add(Keys.keyGenerateUser, mt.methodGenerateNomeUsuario);
		tags.add(Keys.keyListCor, mt.methodListCor);
		tags.add(Keys.keyGerarTabelaLines, mt.methodGerarTabelaLines);
