...
public class SupervisorioAGVSCadastroZoneTime implements Command {

	private ConvertPAGinHTML cph;
	private String[] files = { PathFilesPAG.cadastroZoneTime };
	...
	public void execute(Request req, Response resp) throws Exception {
		...
		if (!Login.login(req, resp, Login.tokenCadastroZoneTime)) {
		...
		tags.add(Keys.keyGenerateDashboard, mt.methodGenerateDashboardCadastroZoneTime);
		tags.add(Keys.keyGenerateUser, mt.methodGenerateNomeUsuario);
		tags.add(Keys.keyGerarComboboxTags, mt.methodGerarComboboxTags);
		tags.add(Keys.keyGerarTabelaZoneTime, mt.methodGerarTabelaZoneTime);
