  ...
  (function($) {
    var EditableTable = {
        ...
        $row.children('td').each(function(i) {
          ...
          if ($this.hasClass('actions')) {
			...
			if(i == 0) { 
			  ...
			  $this.html('<select  id="selectTag' + i +'" required class="form-control" style="width: 100%;" data-plugin="select">' + rowTags() + '</select>');
              tg1 = data[i];
			} else if(i == 3 ) {
			  $this.html('<select  id="selectTag' + i +'" required class="form-control" style="width: 100%;" data-plugin="select">' + rowTags() + '</select>');
              tg2 = data[i];
			} else if(i == 4 ) {
				$this.html('<input type="text" required id="' + i + '" pattern="^([0-9]?[0-9])(:[0-5][0-9])?$" class="form-control input-block" style="width: 100%;" value="' + data[i] + '"/>');
			} else {
              $this.html('<input type="text" required id="' + i + '" class="form-control input-block" style="width: 100%;" value="' + data[i] + '"/>');
            }
			
			$('#4').mask('99:99');
          }
        });
		var element = document.getElementById('selectTag2');
		if(tg1 != '') {
			var i;
			for(i = 0; i < element.childNodes.length; i++) {
				if(element.childNodes[i].text == tg1) {
					element.value = element.childNodes[i].value;
				}
			}
		}
		var element = document.getElementById('selectTag3');
		if(tg2 != '') {
			var i;
			for(i = 0; i < element.childNodes.length; i++) {
				if(element.childNodes[i].text == tg2) {
					element.value = element.childNodes[i].value;
				}
			}
		}
      ...
      rowSave: function($row) {
		...
		var link = '/ActionCadastroZoneTime?action=' + action 
					+ '&id=' + document.getElementById("0").value
					+ '&descricao=' + document.getElementById("1").value 
					+ '&tagStart=' + document.getElementById("selectTag2").value 
					+ '&tagEnd=' + document.getElementById("selectTag3").value 
					+ '&tempo=' + document.getElementById("4").value;
	        ...
	        values = $row.find('td').map(function() {
	          ...
	          if ($this.hasClass('actions')) {
				...
				if(i == 1) {
					...
					e = document.getElementById("selectTag2");
					return e.options[e.selectedIndex].text;
				}else if (i == 4){
					e = document.getElementById("selectTag3");
					return e.options[e.selectedIndex].text;
				}
      ...
      rowRemove: function($row) {
		...
		var link = "/ActionCadastroZoneTime?action=delete&id=" + id;
		if(action == "delete") {
			if(enviarDados(link)) {
